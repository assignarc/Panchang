...
namespace mhora
{
	...
	public class BasicCalculationsControl : MhoraControl
	{
		...
		public BasicCalculationsControl(Horoscope _h)
		{
			...
			this.options.DivisionType = new Division(DivisionType.Rasi);
		...
		private void RepopulateCharaKarakas ()
		{
			...
			for (int i=0; i<al.Count; i++)
			{
				...
				li.Text =org.transliteral.panchang.Body.ToString(bk.GetPosition.name);
				if (this.vt == ViewType.ViewCharaKarakas)
		...
		private void Repopulate64NavamsaHelper (org.transliteral.panchang.Body.Name b, string name, BodyPosition bp, Division div)
		{
		...
		private void Repopulate64Navamsa()
		{
			...
			org.transliteral.panchang.Body.Name[] bodyReferences = new org.transliteral.panchang.Body.Name[]
			{org.transliteral.panchang.Body.Name.Lagna,org.transliteral.panchang.Body.Name.Moon,org.transliteral.panchang.Body.Name.Sun };
			...
			foreach (org.transliteral.panchang.Body.Name b in bodyReferences)
			{
				...
				this.Repopulate64NavamsaHelper (b, "22nd Drekkana (Jagannath)", bp, new Division(DivisionType.DrekkanaJagannath));
			}
		...
		private void RepopulateAvasthas()
		{
			...
			for (int i=(int)org.transliteral.panchang.Body.Name.Sun; i<=(int)org.transliteral.panchang.Body.Name.Ketu; i++)
			{
				org.transliteral.panchang.Body.Name b = (org.transliteral.panchang.Body.Name)i;
				ListViewItem li = new ListViewItem();
		...
		private bool CheckBodyForCurrentView (BodyPosition bp)
		{
			switch (this.vt)
			{
				case ViewType.ViewMrityuLongitudes:
					if (bp.name ==org.transliteral.panchang.Body.Name.MrityuPoint) return true;
					return false;
				...
				case ViewType.ViewBasicGrahas:
					if (bp.name ==org.transliteral.panchang.Body.Name.MrityuPoint ||
						bp.name ==org.transliteral.panchang.Body.Name.Other)
						return false;
					return true;
			}
			return true;
		...
		private void RepopulateBasicGrahas ()
		{
			...
			foreach (BodyPosition bp in h.positionList)
			{
				...
				if ((int)bp.name >= (int)org.transliteral.panchang.Body.Name.Sun && (int)bp.name <= (int)org.transliteral.panchang.Body.Name.Rahu)
